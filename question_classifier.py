...
class QuestionClassifier:
    def __init__(self):
        ...
        #加载特征词
        self.disease_wds = [i.strip() for i in open(self.disease_path) if i.strip()] # words
    ...
    def classify(self, question):
        '''分类主函数
        1.首先，通过标准词库构建字典，{'word': 'type'}，例如{'表征': 'symptom'}
        2.其次，返回句子中包含哪些在标准字典中的词，剔除停用词以后提取出来，这里采用了有穷自动机的方式查找标准词，没有进行分词。
        3.最后，通过某一类关键词是否在用户问句中确定问句的类型，也就是问句问的是什么
        '''
        data = {}
        medical_dict = self.check_medical(question)
        if not medical_dict:
            return {}
        data['args'] = medical_dict #保存问句中主体的类型，也就是实体的类型。
        #收集问句当中所涉及到的实体类型
        types = []
        for type_ in medical_dict.values():
            types += type_
        question_type = 'others'
        question_types = [] #保存问句的类别，根据特征词取判断，依据类别确定cypher语句的编写。
        
        # 症状
        if self.check_words(self.symptom_qwds, question) and ('disease' in types): #根据症状特征词判断问句的类别
    ...
    def check_medical(self, question):
        ...
        return final_dict #返回句子中除去停用词以后，每一个词的类型
